function preload() {
  ...
  function create() {
    ...
    const endGame = () => {
      ...
      this.add.text(150, 250, 'Game Over... \n Click to restart', { fontSize: '17px', fill: '#000' }
  )};
    this.physics.add.collider(pellets, gameState.player, endGame);
  
    gameState.bugRepellent = this.physics.add.group();
    this.physics.add.collider(gameState.bugRepellent, gameState.enemies, function(bug, repellent) {
      bug.destroy();
      repellent.destroy();
      gameState.enemyVelocity += .5;
      gameState.scoreText.setText(`Bugs Left: ${numOfTotalEnemies()}`
    );
  });
  
    this.physics.add.collider(gameState.player, gameState.enemies, endGame);
  }
  ...
  function update() {
      ...
      if (numOfTotalEnemies() === 0) {
        ...
        this.physics.pause();
        gameState.enemyVelocity = 1;
        this.add.text(170, 200, 'You Win!!!', { fontSize: '20px', fill: '#000' });
      } else {
        gameState.enemies.getChildren().forEach(function(bug) {
